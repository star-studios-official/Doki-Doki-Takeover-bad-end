...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSky:BGSprite = new BGSprite('weeb/weebSky', 'week6', OFFSET_X - (playerXDifference / 2) - 300, 0 - playerYDifference, 0.1, 0.1);
			bgLayer.add(bgSky);
			bgSky.antialiasing = false;

			var repositionShit = -200 + OFFSET_X - playerXDifference;

			var bgSchool:BGSprite = new BGSprite('weeb/weebSchool', 'week6', repositionShit, -playerYDifference + 6, 0.6, 0.90);
			...
			var bg:BGSprite = new BGSprite('stageback', 'preload', -600 + OFFSET_X - playerXDifference, -300, 0.9, 0.9);
			bgLayer.add(bg);

			var stageFront:BGSprite = new BGSprite('stagefront', 'preload', -650 + OFFSET_X - playerXDifference, 500, 0.9, 0.9);
	...
	var TemplateCharacter:String = '{
			...
			"sing_duration": 6.1,
			"scale": 1,
			"gameover_character": "gameover-generic",
			"death_sound": "fnf_loss_sfx",
			"win_sound": "dah"
		}';
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.playerIcon = parsedJson.playericon;
				character.costumes = parsedJson.costumes;
				character.costumeArray = parsedJson.costumelist;
				character.healthColorArray = parsedJson.healthbar_colors;
				character.gameoverchara = parsedJson.gameover_character;
				character.gameovercharamirror = parsedJson.gameover_character_mirror;
				character.deathsound = parsedJson.death_sound;
				character.winsound = parsedJson.win_sound;
	...
	function addCharacterUI() {
		...
		imageInputText = new FlxUIInputText(15, 30, 200, 'characters/DADDY_DEAREST', 8);
		...
		noAntialiasingCheckBox.callback = function() {
			...
			if(!noAntialiasingCheckBox.checked && SaveData.globalAntialiasing) {
	...
	var costumesInputText:FlxUIInputText;
	var playerIconInputText:FlxUIInputText;
	var gameoverCharacterInputText:FlxUIInputText;
	var gameoverCharacterMirrorInputText:FlxUIInputText;
	var winSoundInputText:FlxUIInputText;
	var deathSoundInputText:FlxUIInputText;

	function addExtrasUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Extras";

		costumesInputText = new FlxUIInputText(15, 30, 75, 'dad', 8);
		playerIconInputText = new FlxUIInputText(costumesInputText.x + 125, costumesInputText.y, 75, leHealthIcon.getCharacter(), 8);
		gameoverCharacterInputText = new FlxUIInputText(15, costumesInputText.y + 35, 200, char.gameoverchara, 8);
		gameoverCharacterMirrorInputText = new FlxUIInputText(15, gameoverCharacterInputText.y + 35, 200, char.gameovercharamirror, 8);
		winSoundInputText = new FlxUIInputText(15, gameoverCharacterMirrorInputText.y + 35, 100, char.winsound, 8);
		deathSoundInputText = new FlxUIInputText(15, winSoundInputText.y + 35, 100, char.deathsound, 8);

		tab_group.add(new FlxText(15, costumesInputText.y - 18, 0, 'Costumes name:'));
		tab_group.add(new FlxText(costumesInputText.x + 125, playerIconInputText.y - 18, 0, 'Player icon name:'));
		tab_group.add(new FlxText(15, gameoverCharacterInputText.y - 18, 0, 'Gameover Character name:'));
		tab_group.add(new FlxText(15, gameoverCharacterMirrorInputText.y - 18, 0, 'Mirror Gameover Character name:'));
		tab_group.add(new FlxText(15, winSoundInputText.y - 18, 0, 'Win Sound name:'));
		tab_group.add(new FlxText(15, deathSoundInputText.y - 18, 0, 'Death Sound name:'));
		tab_group.add(costumesInputText);
		tab_group.add(playerIconInputText);
		tab_group.add(gameoverCharacterInputText);
		tab_group.add(gameoverCharacterMirrorInputText);
		tab_group.add(winSoundInputText);
		tab_group.add(deathSoundInputText);
		UI_characterbox.addGroup(tab_group);
	}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == costumesInputText) {
				char.costumes = costumesInputText.text;
			}
			else if(sender == playerIconInputText) {
				char.playerIcon = playerIconInputText.text;
			}
			else if(sender == gameoverCharacterInputText) {
				char.gameoverchara = gameoverCharacterInputText.text;
			}
			else if(sender == gameoverCharacterMirrorInputText) {
				char.gameovercharamirror = gameoverCharacterMirrorInputText.text;
			}
			else if(sender == winSoundInputText) {
				char.winsound = winSoundInputText.text;
			}
			else if(sender == deathSoundInputText) {
				char.deathsound = deathSoundInputText.text;
			}
	...
	function reloadCharacterDropDown() {
		...
		characterList = [];
		var directory:String = Paths.getPreloadPath('data/characters/');
	...
	override function update(elapsed:Float)
	{
		...
		/*var inputTexts:Array<FlxUIInputText> = [animationInputText, imageInputText, healthIconInputText, animationNameInputText, animationIndicesInputText, costumesInputText, playerIconInputText];
	...
	function saveCharacter() {
		var json = {
			...
			"playericon": char.playerIcon,
			"costumes": char.costumes,
			"costumelist": char.costumeArray,
			...
			"gameover_character": char.gameoverchara,
			"gameover_character_mirror": char.gameovercharamirror,
			"death_sound": char.deathsound,
			"win_sound": char.winsound
		};
